...
describe CoursesHelper do
  it "if course is unactive" do
    ...
    index = 0
    expect(helper.course_link(course, index)).to eq(
      content_tag(:span, "#{index}: #{course.title}",
        data: { toggle: 'tooltip', placement: 'bottom' },
        title: 'Этот курс еще не готов, но если вы оставите нам ваш email, мы пришлем вам письмо, когда он станет доступен!'))
  end
